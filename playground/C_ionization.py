...
for dens in jnp.logspace(-1, 3, 350):
    ...
    n_e_saha.append(state.n_e.m_as(ureg.centimeter ** (-3)))

    charge_distribution, Z_mean = jaxrts.saha.calculate_mean_free_charge_saha(
        state, True, True
    )
    Z_gen_saha.append(Z_mean)
    n_e_gen_saha.append(state.n_e.m_as(ureg.centimeter ** (-3)))
    ...
    Z_BU.append(Z_mean)
    n_e_BU.append(state.n_e.m_as(ureg.centimeter ** (-3)))

plt.xlabel("$n_e$ (1/cc)")
plt.ylabel("$Z$")
plt.title("C, SP, 100eV")

plt.plot(n_e_saha, Z_saha, label="Saha")
plt.plot(n_e_gen_saha, Z_gen_saha, label="gen Saha")
plt.plot(n_e_BU, Z_BU, label="BU")
plt.xscale("log")
plt.legend()
plt.show()
