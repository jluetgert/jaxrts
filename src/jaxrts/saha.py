...
def planck_larkian_partition_sum(
    g: jnp.ndarray, E: Quantity, T: Quantity
) -> jnp.ndarray:
    """
    See :cite:`Kramp.2005` eqn 6.182, with a cutoff for negative energies
    """
    ratio = (E / (T * ureg.boltzmann_constant)).m_as(ureg.dimensionless)
    return jnp.sum(g * (jnp.exp(-ratio) - 1 + ratio) * jnp.heaviside(ratio, 0))
...
def solve_BU(
    ...
    """
    Solve the BU equation for a list of elements at a given temperature.
    ...
    #. Creating an abstract matrix which for each element consists of
        ...
        #. `Z + 1` rows of which have the solution of the BU equation as
           diagonal entries, multiplied by -i and the free electron density
           `n_e` on the first off-diagonal element. These rows reflect that
           every neighboring number densities `n_{i+1}` and `n_i` fulfill
           the BU equation
    ...
    continuum_lowering: Quantity, default: 0 eV
        ...
        0 eV.
    chem_pot_ideal: Quantity, default: 0 eV
        The ideal chemical potential used in the BU equation.
    ...
    for element, ipd in zip(element_list, continuum_lowering, strict=True):
        pls = []
        for charge, cl in zip(range(element.Z), ipd, strict=True):
            g, E = read_nist_file(f"{element.symbol}{charge}")
            E += cl
            pls.append(planck_larkian_partition_sum(g, E, T_e))
        pls.append(1)
        part_funcs.append(jnp.array(pls))

    all_part_funcs = jnp.concatenate(part_funcs)

    nom = jnp.heaviside(all_part_funcs, 0)
    ratio = jnp.sum(nom / len(all_part_funcs))
    ...
    # Note: The matrix is transposed, here. It will be transposed when n_e is
    # inserted.
    for ion_dens, element, part_func in zip(
        ion_number_densities, element_list, part_funcs, strict=True
    ):
        coeff = (
            gen_balance_equation(
                part_func[:-1],
                part_func[1:],
                T_e,
                n_e,
        ...
        diag = jnp.diag(jnp.where(part_func[:-1] > 0, (-1) * coeff, 1))
    ...
    def insert_ne(M, ne):
        ...
        ne_line = jnp.ones(len(element_list) + int(onp.sum(Z))) * ne
        skip = -1
        for element, part_func in zip(element_list, part_funcs, strict=True):
            # Not the full off-diagonal equals n_e: The density rows don't
            # contain it!
            ne_line = ne_line.at[skip + 1 : skip + element.Z + 1].multiply(
                    jnp.heaviside(part_func[:-1], 0)
            )
            ne_line = ne_line.at[skip + element.Z + 1].set(0.0)
            skip += element.Z + 1
...
def calculate_mean_free_charge_BU(
    ...
    """
    Calculates the mean charge of each ion in a plasma using the BU
    equation.
    ...
    exclude_non_negative_energies : bool, default = True
        ...
        their Boltzmann factors in the BU equations.
    ...
    jaxrts.saha.solve_BU
        Function used to solve the BU equation
    """
    ...
    charge_distribution, ne, Z_mean = solve_BU(
        tuple(plasma_state.ions),
        plasma_state.T_e,
        plasma_state.n_e,
        (plasma_state.mass_density / plasma_state.atomic_masses),
